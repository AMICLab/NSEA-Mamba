...
if True:
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_g: torch.Tensor, x_e: torch.Tensor):
            # B, C, H, W -> B, 2, C, 2 * H * W
            B, C, H, W = x_g.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_g.new_empty((B, 2, C, 2 * H * W))
            xs_fuse[:, 0] = torch.concat([x_g.flatten(2, 3), x_e.flatten(2, 3)], dim=2)
    ...
    def cross_selective_scan_multimodal_k1(
        ...
        # x_fuse = CrossScan_multimodal.apply(x_g, x_e) # B, C, H, W -> B, 1, C, 2 * H * W
        B, C, H, W = x_g.shape
        x_fuse = x_g.new_empty((B, 1, C, 2 * H * W))
        x_fuse[:, 0] = torch.concat([x_g.flatten(2, 3), x_e.flatten(2, 3)], dim=2)
        ...
        if softmax_version:
            y = y.softmax(y, dim=-1).to(x_g.dtype)
            y = y.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        else:
            y = y.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
            y = out_norm(y).to(x_g.dtype)
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        x_fuse = CrossScan_multimodal.apply(x_g, x_e)  # B, C, H, W -> B, 2, C, 2 * H * W
        ...
        y_g, y_e = CrossMerge_multimodal.apply(ys)

        y_g = y_g.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_e = y_e.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_g = out_norm1(y_g).to(x_g.dtype)
...
class ConMB_SS2D(nn.Module):
    ...
    def forward_corev2_multimodal(self, x_g: torch.Tensor, x_e: torch.Tensor, nrows=-1):
        return cross_selective_scan_multimodal_k2(
            x_g, x_e, self.x_proj_weight, None, self.dt_projs_weight, self.dt_projs_bias,
            self.A_logs, self.Ds, getattr(self, "out_norm1", None), getattr(self, "out_norm2", None),
            self.softmax_version,
            nrows=nrows,
        )

    def forward(self, x_g: torch.Tensor, x_e: torch.Tensor):
        x_g = self.in_proj(x_g)
        x_e = self.in_proj_modalx(x_e)
        if self.d_conv > 1:
            x_g_trans = x_g.permute(0, 3, 1, 2).contiguous()
            x_e_trans = x_e.permute(0, 3, 1, 2).contiguous()
            x_g_conv = self.act(self.conv2d(x_g_trans))  # (b, d, h, w)
            x_e_conv = self.act(self.conv2d_modalx(x_e_trans))  # (b, d, h, w)
            y_g, y_e = self.forward_corev2_multimodal(x_g_conv, x_e_conv)  # b, d, h, w -> b, h, w, d
            # SE to get attention, scale
            b, d, h, w = x_g_trans.shape
            x_g_squeeze = self.avg_pool(x_g_trans).view(b, d)
            x_e_squeeze = self.avg_pool(x_e_trans).view(b, d)
            x_g_exitation = self.fc1(x_g_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()  # b, 1, 1, d
            x_e_exitation = self.fc2(x_e_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()
            y_g = y_g * x_e_exitation
            y_e = y_e * x_g_exitation
            y = torch.concat([y_g, y_e], dim=-1)
        out = self.dropout(self.out_proj(y))
        return out
...
class Cross_Mamba_Attention_SSM(nn.Module):
    ...
    def forward(self, x_g: torch.Tensor, x_e: torch.Tensor):
        ...
        x_dbl_g = self.x_proj_1(rearrange(x_g, "b d l -> (b l) d"))  # (bl d)
        ...
        dt_g = self.dt_proj_1.weight @ dt_g.t()
        dt_e = self.dt_proj_2.weight @ dt_e.t()
        dt_g = rearrange(dt_g, "d (b l) -> b d l", l=L)
        dt_e = rearrange(dt_e, "d (b l) -> b d l", l=L)
        A_g = -torch.exp(self.A_log_1.float())  # (k * d, d_state)
        A_e = -torch.exp(self.A_log_2.float())  # (k * d, d_state)
        B_g = rearrange(B_g, "(b l) dstate -> b dstate l", l=L).contiguous()
        B_e = rearrange(B_e, "(b l) dstate -> b dstate l", l=L).contiguous()
        C_g = rearrange(C_g, "(b l) dstate -> b dstate l", l=L).contiguous()
        C_e = rearrange(C_e, "(b l) dstate -> b dstate l", l=L).contiguous()

        y_g = selective_scan(
            x_g, dt_g,
            A_g, B_g, C_e, self.D_1.float(),
            delta_bias=self.dt_proj_1.bias.float(),
            delta_softplus=True,
        )
        y_e = selective_scan(
            x_e, dt_e,
            A_e, B_e, C_g, self.D_2.float(),
            delta_bias=self.dt_proj_2.bias.float(),
            delta_softplus=True,
        )
        # assert out_y.dtype == torch.float
        y_g = rearrange(y_g, "b d l -> b l d")
        y_e = rearrange(y_e, "b d l -> b l d")
        y_g = self.out_norm_1(y_g)
...
class CrossMambaFusion_SS2D_SSM(nn.Module):
    def __init__(self, d_model=96, d_state=16, ssm_ratio=2, dt_rank="auto", d_conv=3, conv_bias=True, dropout=0., bias=False, dt_min=0.001, dt_max=0.1, dt_init="random", dt_scale=1.0, dt_init_floor=1e-4, softmax_version=False, **kwargs):
        super(CrossMambaFusion_SS2D_SSM, self).__init__()
        self.softmax_version = softmax_version
        self.d_model = d_model
        ...
        self.in_proj_modalx = nn.Linear(self.d_model, self.d_inner, bias=bias)  # 确保添加了这一行
        ...
        self.out_proj_g = nn.Linear(self.d_inner, self.d_model, bias=bias)
        self.out_proj_e = nn.Linear(self.d_inner, self.d_model, bias=bias)
        self.dropout_g = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
    ...
    def forward(self, x_g: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            x_g_conv = rearrange(x_g_conv, "b d h w -> b (h w) d")
            x_e_conv = rearrange(x_e_conv, "b d h w -> b (h w) d")
            y_g, y_e = self.CMA_ssm(x_g_conv, x_e_conv)
            # to b, d, h, w
            y_g = y_g.view(B, H, W, -1)
            y_e = y_e.view(B, H, W, -1)

        out_g = self.dropout_g(self.out_proj_g(y_g))
...
class CVSSDecoderBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        # print(f"Input shape before error: {input.shape}")
...
class CrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_g: torch.Tensor, x_e: torch.Tensor):
        x_g_cross, x_e_cross = self.op(x_g, x_e)
        x_g = x_g + self.drop_path1(x_g_cross)
    ...
    def forward(self, x_g: torch.Tensor, x_e: torch.Tensor):
        ...
        '''
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, x_g, x_e)
        else:
            return self._forward(x_g, x_e)
...
class SomeModule(nn.Module):
    def _forward(self, x_g, x_e):
        result = self.op(x_g, x_e)
        print("Result of self.op:", result)
        print("Type of result:", type(result))
        print("Number of values returned:", len(result) if isinstance(result, (list, tuple)) else "Single value")
        # 根据返回值的数量进行解包
        if isinstance(result, (list, tuple)):
            x_g_cross, x_e_cross = result[0], result[1]
        else:
            x_g_cross, x_e_cross = result, None
        return x_g_cross, x_e_cross
...
class VSSM(nn.Module):
    ...
    def flops(self, shape=(1, 180, 180)):
...
class Backbone_VSSM(VSSM):
    def __init__(self, patch_size=4, in_chans=1,
